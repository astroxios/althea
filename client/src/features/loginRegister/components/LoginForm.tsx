      ...
      <form onSubmit={handleSubmit((data) => submitLoginForm(data))} className="min-h-[543px] max-w-md w-full bg-white shadow-lg py-11 px-16 mx-5 rounded-2xl">
        ...
        <div className="flex flex-col gap-5 mt-px min-w-28">
          <div className="flex flex-col gap-2">
            <Input {...register("emailOrUsername")} type="text" placeholder="Email / Username" className={`w-full ${errors.emailOrUsername?.message ? "outline outline-rose outline-2 focus:outline-rose" : "focus:outline-oceanBlue"} outline-offset-0 focus:outline-offset-0`} />
            {errors.emailOrUsername && (
              <div className="flex items-center justify-center gap-2">
                <img src={Caution} alt="caution icon" className="w-auto h-4" />
                <p className="text-red-400 text-sm text-center">{`${errors.emailOrUsername.message}`}</p>
              </div>
            )}
          </div>
          <div className="grid gap-2">
            <Input type="password" placeholder="Password" className={`w-full ${errors.password?.message ? "outline outline-rose outline-2 focus:outline-rose" : "focus:outline-oceanBlue"} outline-offset-0 focus:outline-offset-0`} />
            {errors.password && (
              <div className="flex items-center justify-center gap-2">
                <img src={Caution} alt="caution icon" className="w-auto h-4" />
